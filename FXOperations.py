...
file = symbol.replace("/", "_") + "_" + args.timeframe + ".csv"
...
def getLatestPriceData():
    ...
    with ForexConnect() as fx:
        try:
            fx.login(args.userid, args.password, args.url, session_status_callback=session_status_changed)
            args = ConfigurationOperation()
            history = fx.get_history(args.instrument, args.timeframe, args.date_from, args.date_to)
...
def Update(pricedata):
    print(str(dt.datetime.now()) + " " + args.timeframe + " Bar Closed " + symbol)
    ...
    if df['zone_sell'][len(df) - 5] == -1:
        args.d_buy_sell = "B"
        CloseOperation(BuySell = args.d_buy_sell)


    if df['zone_sell'][len(df) - 5] == 1:
        args.d_buy_sell = "S"
        if existingOperation(instrument=args.instrument, BuySell= args.d_buy_sell)  != True:
            createEntryOrder()
    ...
    print(str(dt.datetime.now()) + " " + args.timeframe +  " Update Function Completed. " + symbol + "\n")
